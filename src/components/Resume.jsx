...
import { MapPin, Mail, ExternalLink, Download, X, Facebook, Github, Printer } from 'lucide-react';
import html2pdf from 'html2pdf.js';

const ResumeContent = ({ profileMain, fallbackProfile, id }) => (
  <div id={id} className="w-[794px] p-8 sm:p-12 bg-white text-slate-900 box-border text-left flex flex-col font-sans" style={{ minHeight: '1123px' }}>
    {/* Header */}
    ...
    <section className="mb-6">
      ...
      <div className="mb-4 print:break-inside-avoid avoid-break">
      ...
      <div className="mb-2 print:break-inside-avoid avoid-break">
        <div className="flex justify-between items-end mb-1">
    ...
    <div className="grid grid-cols-2 gap-8 mb-6 print:break-inside-avoid avoid-break" style={{ pageBreakInside: 'avoid' }}>
      {/* Technical Skills */}
      <section>
        <h3 className="text-lg font-bold text-blue-800 uppercase mb-3 flex items-center gap-2">
    ...
    {/* Signature Section */}
    <section className="mt-auto flex justify-between items-end pt-8 pb-4 border-t-2 border-slate-100 print:break-inside-avoid avoid-break" style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
      <div>
      ...
      <div className="text-right flex flex-col items-center">
        <div className="font-signature text-4xl text-blue-800 transform -rotate-2">MD SIAM</div>
...
const Resume = ({ isResumeModalOpen, setIsResumeModalOpen, profileMain, fallbackProfile }) => {
  ...
  return (
    <>
      <section id="resume" className="py-20 bg-slate-900 border-t border-slate-800 relative overflow-hidden no-print">
        ...
        <div className="fixed inset-0 z-[200] bg-slate-900/95 backdrop-blur-md overflow-y-auto no-print">
          <div className="min-h-full flex justify-center p-4 md:py-10">
            <div className="w-full max-w-[210mm] flex flex-col items-center animate-in fade-in zoom-in duration-300">
              ...
              {/* Controls Header */}
              <div className="flex flex-col sm:flex-row justify-between items-center w-full bg-slate-800 p-4 rounded-2xl border border-slate-700 mb-6 sticky top-4 z-10 shadow-2xl gap-4">
                <div className="text-white font-medium text-sm hidden md:block">Previewing Resume</div>
                <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                  <button onClick={handleDownloadPDF} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium flex items-center gap-2 font-fira transition-colors shadow-lg shadow-blue-600/20 text-sm">
                    <Download size={18} /> Download PDF
                  </button>
                  <button onClick={() => window.print()} className="px-4 py-2 bg-slate-600 hover:bg-slate-700 text-white rounded-lg font-medium flex items-center gap-2 font-fira transition-colors shadow-lg shadow-slate-600/20 text-sm">
              ...
              <div className="w-full overflow-x-auto bg-slate-100/50 p-4 rounded-xl border border-slate-700/50 flex justify-center">
                <div className="shadow-2xl bg-white border border-slate-300 shrink-0">
                  <ResumeContent id="cv-print-content" profileMain={profileMain} fallbackProfile={fallbackProfile} />
                </div>
              </div>
            </div>
          </div>
